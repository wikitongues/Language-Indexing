...
API
    ...
    def get_fields_dict(self, item):
        ...
        if 'resource_languages_raw' in item:
            fields[field_name.RESOURCE_LANGUAGES_RAW_FIELD] = ','.join(item['resource_languages_raw'])

        if 'resource_languages' in item:
            fields[field_name.RESOURCE_LANGUAGES_LOOKUP_FIELD] = list(item['resource_languages'])
