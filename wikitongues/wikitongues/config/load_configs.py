...
def read_include_languages(config):
    if hasattr(config, 'include_language') and len(config["include_languages"].__dict__) > 0:
        return config['include_languages']['include_languages'].split(",")
    return None


def read_exclude_languages(config):
    if hasattr(config, 'include_language') and len(config["exclude_languages"].__dict__) > 0:
