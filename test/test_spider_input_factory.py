...
import wikitongues.wikitongues.config.config_keys as keys
from wikitongues.wikitongues.language_indexing_config import (
    LanguageIndexingConfiguration,
    Title,
)
from wikitongues.wikitongues.spider_input_factory import SpiderInputFactory
from wikitongues.wikitongues.spiders.translated_site_spider import (
    TranslatedSiteSpiderInput,
)
from wikitongues.wikitongues.spiders.wikipedia_spider import WikipediaSpiderInput

MODULE_UNDER_TEST = "wikitongues.wikitongues.spider_input_factory"
...
class TestSpiderInputFactory(unittest.TestCase):
    def setUp(self) -> None:
        ...
        setattr(
            self.configs.config_languages_table,
            keys.MAX_RECORDS_KEY,
            EXPECTED_MAX_RECORDS,
        )

        setattr(self.configs.main_config, keys.TRANSLATED_SITE_URLS_SECTION, Title())
        setattr(
            self.configs.main_config[keys.TRANSLATED_SITE_URLS_SECTION],
            BBC_KEY,
            BBC_URL,
        )
        setattr(self.configs.main_config, keys.TRANSLATED_SITE_SELECTORS_SECTION, Title())
        setattr(
            self.configs.main_config[keys.TRANSLATED_SITE_SELECTORS_SECTION],
            BBC_KEY,
            BBC_SELECTOR,
        )
